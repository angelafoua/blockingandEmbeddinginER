...
\author{%
\IEEEauthorblockN{Lou Angela Foua and Muzzakiruddin Ahmed Mohammed}
\IEEEauthorblockA{Department of Computer Science\\
University of Arkansas at Little Rock\\
Little Rock, Arkansas, USA\\
\{ldfoua1, mmohammed6\}@ualr.edu}
}
...
\midrule
S1G   & .964 & 1.000 & .9818 & $\tau=.30$, m0p0 \\
S2G   & .917 & .917  & .9167 & Full, m0p0 \\
S4G   & .941 & .831  & .8826 & $\delta=.70$, m1p0 \\
S5G   & .873 & .900  & .8861 & Full, m1p0 \\
S7GX  & .871 & .903  & .8863 & Full, m1p0 \\
S8P   & .553 & .506  & .5281 & $\tau=.05$, m1p0 \\
S12PX & .664 & .306  & .4185 & $\tau=.05$, m1p0 \\
S14GX & .923 & .839  & .8788 & Full, m1p0 \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:lambda} isolates the pair-adaptation sweep on S8P for the merge-only cell. Block-only weighting ($\lambda=0$) yields high recall but extremely low precision. Moving to $\lambda=.50$ raises F1 from .0662 to .3940, and $\lambda=.75$ produces the best value in this sweep, .4080. Full modulation ($\lambda=1$) raises precision further but loses too much recall. Thus, the pair term in (\ref{eq:pairweight}) is not simply a monotonic quality control; it moves the operating point from permissive block evidence toward stricter direct similarity.

\begin{table}[t]
\caption{Pair-Adaptation Sweep on S8P (m1p0)}
\label{tab:lambda}
\centering
\scriptsize
\begin{tabular}{crrr}
...
\begin{table}[t]
\caption{Edge-Threshold Sweep on S8P (m1p0)}
\label{tab:tau}
\centering
\scriptsize
\begin{tabular}{crrr}
...
\begin{table}[t]
\caption{Recursive-Depth Sweep on S8P (m0p0)}
\label{tab:depth}
\centering
\scriptsize
\setlength{\tabcolsep}{3.2pt}
...
\begin{table}[t]
\caption{Filter and Density Sweeps on S8P (m1p0)}
